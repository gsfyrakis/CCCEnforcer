...
/**
 * The Class EventLogger. Instances of this class represent an Event Logger
 * entity that receives events, stores them in a database and answers queries
 * about them.
 */
public class EventLogger {
    ...
    public EventLogger(String dbLocation, String username, String password) {
        ...
        if ((dbLocation == null) || (dbLocation.length() == 0))
            throw new IllegalArgumentException(
                    "Database location is null or empty");
        ...
        try {
            ...
            ErrorMessageManager
                    .fatalErrorMsg("Connection NOT established - maybe the server is down?");
        }
    ...
    /**
     * Establish connection. Obtain Connection object to database (static)
     *
     * @param dbLocation the db location
     * @param username   the username
     * @param password   the password
    ...
    private static Connection establishConnection(String dbLocation,
        ...
        Connection c = null;
        try {
            c = DriverManager.getConnection("jdbc:mysql:" + dbLocation
                    + "?user=" + username + "&password=" + password);
        } catch (Exception e) {
            ErrorMessageManager.errorMsg(
                    "Generation of Connection object failed", e);
            c = null;
            throw e;
        }
        return c;
    ...
    /**
     * Execute historical query. Perform historical (SELECT) query
     *
     * @param query the query
    ...
    private boolean executeHistoricalQuery(String query) {
        try {
            ...
            ErrorMessageManager.errorMsg(
                    "Exception occurred while executing historical query"
                            + "\n  " + query, e);
            return false;
        }
    }
    ...
    /**
     * Perform counting historical query Execute counting query.
     *
     * @param query the query
     * @return the int
     */
    ...
    public String toString() {
        return new String("Event Logger - DB Location:" + dbLocation
                + ", Username:" + username + ", Connection:" + conn);
    }
     ...
     * Happened.
     * <p>
     * Query the historical database NOTE: I assume that the String
     * timeConstraints is in acceptable format an SQL query.
     *
    ...
    public boolean happened(String eventType, String timeConstraints,
        ...
        // Build query
        String query = "SELECT * FROM eventhistory WHERE " + "type='"
                + eventType + "' AND " + "originator='" + originator + "' AND ";
        ...
        query = query.concat("responder='" + responder + "' AND " + "status='"
                + status + "'");
    ...
    /**
     * Count happened. Query the historical database NOTE: I assume that the
     * String timeConstraints is in acceptable
     *
     * @param eventType       the event type
     * @param timeConstraints the time constraints
     * @param originator      the originator
    ...
    public int countHappened(String eventType, String timeConstraints,
        ...
        if ((eventType == null)
            ...
            throw new IllegalArgumentException("Null or empty arguments passed");
        // Build query
        String query = "SELECT COUNT(*) FROM eventhistory WHERE " + "type='"
                + eventType + "' AND " + "originator='" + originator + "' AND ";
        // TODO: Parsing of time constraints
        // If timeConstraints is empty or null, no filtering by timestamp
        if ((timeConstraints != null) && (timeConstraints.length() > 0))
